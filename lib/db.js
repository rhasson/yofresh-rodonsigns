...
DB.prototype.save = function(type, user_id, item) {
	...
	if (type === 'user') item.password = calc(item.password);
	item.type = type;
	item.created_at = new Date().toJSON();
	item.modified_at = new Date().toJSON();
...
calc = function(password) {
	var salt = 'this is a seed for db salting of passwords';
	/*
		1. base64 decode password hash
		2. concatenate password with salt hash
		3. hash new password

/*
* DB abstraction for the storing and loading records
* v.0.0.2
*/
...
DB.prototype.auth = function(email, password) {
	...
	if (!email || email === '' || !password || password === '') def.reject(new Error('Email or password is missing'));
	Q.ninvoke(self._db, 'view', 'users/by_email', {key: email})
	.then(function(doc) {
		if (doc.length && pass === doc[0].value.password) {
...
* @user_id: user id to get record for
* @type: type of record to get
* @params:
...
*/
DB.prototype.get_by_userid = function(type, user_id, params) {
	var self = this
		, couch_obj = {}
		, def = Q.defer()
		, view = type + '/by_userid';
	...
	couch_obj = params || {};
	couch_obj.key = user_id;
	couch_obj.sort = couch_obj.sort || 'desc';

	if (couch_obj.count) couch_obj.limit = couch_obj.count;
...
/*
* Save record to redis and couch
...
* update item in db
* @type: document type
* @id: the doc id for which to save
